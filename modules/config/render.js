...
class Chart {
	...
	'updateSeries' (bl = global.baselineLower, bu = global.baselineUpper) {
		...
		return Object.keys(meta.series).filter((k) => {
			...
			try {
				...
				const g = meta.series[key].group;
				// serie.visible = (serie.visible == undefined ? false : true) 
				this.chart.addSeries(serie)
				console.log(serie)

				serie.promises.then((data) => {
					serie.data = data.shortValues
					console.log('update', serie.data)
					$(`#${id}`).highcharts().series[index].update(serie)
					this.chart.redraw()
				})
	...
	"switchToGroup" (gID, changeVisibility = true, change = true) {
		...
		let title = this.title(gID),
			...
			series_count = 0;
		if (change) {
			// if highcharts have no series add Object.keys(meta.series)
